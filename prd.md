...
> **PostgreSQL 实现更新（2026-07-15）。** PostgreSQL 已成为默认生产持久化后端。Alembic 版本 `20260714_01` 创建任务、事件、行程、版本、每日路线、站点和导出记录表；原型 `trips` / `routes` 表会保留为 `legacy_trips` / `legacy_routes`。JSONL 仅作为显式配置的 `TRIP_PERSISTENCE_BACKEND=jsonl` 兼容/测试后端。

> **运行约束。** 所有后端安装、迁移和测试命令在虚拟环境执行。部署数据库前，须在 `backend` 目录配置 `DATABASE_URL` 并运行 `python -m alembic -c alembic.ini upgrade head`，然后再启动 API。

**文档版本：** v6.2
**更新依据：** 当前仓库实现（截至 2026-07-15，含 PostgreSQL 持久化、UI 保存行程导出登记、导出清理状态同步、版本化重规划、P0 稳定性修复、重试/熔断和 Amap MCP Weather 降级修复）
...
| 局部重规划 | 部分实现 | 后端支持 cheapest insertion 单点插入；保存行程重规划会追加不可变版本并校验 `expected_version_id`。前端仍是固定按钮示例，尚无拖拽和版本冲突交互。 |
...
| 保存行程导出登记 | 已实现 | UI 的 `Export File` 自动建立或复用 `saved_trip_id`，导出绑定具体 `trip_version_id`，并记录格式、路径、大小、状态和过期时间。 |
| Job 持久化 | 已实现 | 默认 PostgreSQL 存储（任务、事件、最终状态和关联行程）；JSONL 仅保留为 `TRIP_PERSISTENCE_BACKEND=jsonl` 的测试/兼容回退。健康检查端点可感知持久化异常。 |
...
| **数据生命周期管理** | **已实现** | 数据库任务按年龄/数量清理；导出文件按 mtime/数量清理，并将数据库记录同步为 `expired` 或 `missing`；支持 CLI、REST API 和启动自动清理。 |
| **向后兼容数据迁移** | **已实现** | 仅 JSONL 回退模式下，旧版 `jobs.jsonl` 中无 `created_at` 的记录加载时赋予 epoch-0 时间戳，首次 cleanup 即清除。 |
...
## 2.1 PostgreSQL 持久化与数据契约

### 持久化模式与部署

- 生产默认值为 `TRIP_PERSISTENCE_BACKEND=database`；连接和连接池参数从 `DATABASE_URL`、`TRIP_DB_POOL_SIZE`、`TRIP_DB_MAX_OVERFLOW`、`TRIP_DB_POOL_TIMEOUT_SECONDS`、`TRIP_DB_CONNECT_TIMEOUT_SECONDS` 读取。
- 表结构仅通过迁移变更；应用启动时不再调用 `Base.metadata.create_all()`。
- 部署必须先执行 Alembic 迁移；通过 `TRIP_RUN_DB_INTEGRATION=true` 显式启用数据库专项验证。
- `TRIP_PERSISTENCE_BACKEND=jsonl` 保留旧文件存储，供隔离算法测试和临时回退使用，但不再是生产事实来源。

### 持久化实体

| 实体 | 存储表 | 职责 |
| --- | --- | --- |
| 规划任务 | `planning_jobs` | 保存标准化意图、状态、最终状态快照、错误、生命周期时间戳和已保存行程引用。 |
| 规划事件 | `planning_job_events` | 按 `(job_id, sequence)` 有序回放事件，保持既有 `after` 轮询契约。 |
| 已保存行程 | `trips` | 保存面向用户的行程元数据和当前版本指针。 |
| 行程版本 | `trip_versions` | 保存不可变的 `TripState` / 求解快照；首次完成任务创建版本 1，后续保存重规划追加版本并记录父版本。 |
| 每日路线与站点 | `trip_days`、`trip_stops` | 保存可查询的每日/POI 投影，并以 JSONB 保留完整源数据。 |
| 导出记录 | `trip_exports` | 保存指定行程版本的导出格式、路径、内容类型、文件大小、状态和过期时间。 |

### 已保存行程 API

- `GET /api/trips/saved`：返回当前已保存行程摘要。
- `POST /api/trips/saved`：保存完整 `TripState` 并返回行程 ID、当前版本和状态；用于接管 Plan/Demo/Stream 等尚无持久化身份的 UI 结果。
- `GET /api/trips/saved/{trip_id}`：返回当前版本元数据及完整 `TripState` 快照；不存在或已归档时返回 404。
- `POST /api/trips/saved/{trip_id}/replan`：从数据库读取当前版本，执行局部重规划并追加不可变版本；可传 `expected_version_id`，陈旧版本返回 409。
- `POST /api/trips/saved/{trip_id}/export/file`：导出数据库中的当前版本，并将产物登记到 `trip_exports`。
- `GET /api/trips/saved/{trip_id}/exports`：按时间倒序返回该行程所有版本的导出记录。
- 既有 `/api/trips/jobs*` 契约保持不变；数据库任务完成时会创建并关联一个已保存行程。

UI 的 `Export File` 必须走保存行程接口：数据库 Job 完成事件返回 `saved_trip_id`；直接规划、Demo 或 WebSocket 结果在首次文件导出前调用 `POST /api/trips/saved`。随后统一调用 `/api/trips/saved/{trip_id}/export/file`，因此成功返回的文件必定对应一条 `trip_exports` 记录。无状态 `/api/trips/export/file` 仅保留为兼容接口，不用于 UI 文件导出。

版本追加在事务中锁定 `trips` 行，并同时校验当前版本指针；因此两个客户端基于同一旧版本提交时，仅首个提交成功，后续请求返回 409，不会静默覆盖已保存结果。既有无状态 `/api/trips/replan` 与 `/api/trips/export/file` 保留用于兼容，但不会自动写入行程版本或导出记录。

### 原型数据迁移与验证

- 初始迁移会将旧原型 `trips`、`routes` 表重命名为 `legacy_trips`、`legacy_routes`，不删除历史数据；评估旧记录后可新增专门导入流程。
- 进入 `open_ai` 环境并在 `backend` 目录运行 `python -m alembic -c alembic.ini upgrade head` 后，方可启动数据库模式 API。
- 执行 `TRIP_RUN_DB_INTEGRATION=true python -m pytest tests/test_postgres_integration.py -q` 可验证已完成任务、有序事件回放、行程创建、不可变版本追加、陈旧版本冲突和导出登记；测试只清理自身创建的记录。
- 当前 Alembic 版本为 `20260714_01 (head)`；数据库模型、E2E 冒烟测试与 PostgreSQL 集成测试共 20 项通过。完整 MCP 测试集需要先启动本地 MCP HTTP 服务。
...
8. 对已保存行程的编辑通过版本化重规划接口提交；成功后产生新版本，导出则绑定提交时的具体版本并进入历史列表。
...
- `GET /health`：返回 `{"status": "ok", "app": "Trip Planner", "version": "0.1.0"}`。当任务存储持久化异常时返回 `job_store_warning` 字段。
- `GET /health/capabilities`：返回运行时集成开关（不暴露密钥）。
- `GET /health/integrations/probe`：执行实时 smoke test，分别探测高德和 LLM 可用性。
- `GET /health/job-store`：返回任务存储的 `persistence_error` 和 `job_count`。
...
| 模块 | 清理策略 | 默认阈值 | 触发方式 |
|------|---------|----------|---------|
| 数据库任务存储 | 按 `PlanningJob.created_at` 年龄 | 48h / 200 条 | 启动自动 + REST API + CLI |
| 导出文件及记录 | 文件按 mtime/数量清理；数据库状态对账 | 3 天 / 50 个 | 启动自动 + REST API + CLI |

导出清理以目录为边界执行可重入对账：本次成功删除或已达到 `expires_at` 的 `ready` 记录更新为 `expired`；记录仍为 `ready` 但文件不存在时更新为 `missing`。数据库同步失败会让清理调用失败，下一次运行仍可根据文件存在性和过期时间重新对账。

**向后兼容**：
- 仅 JSONL 回退模式下，旧版 `jobs.jsonl` 记录（无 `created_at` 字段）在 `JobStore._load()` 中自动检测并赋予 `datetime.min（epoch-0）` 时间戳，下一次调用 `cleanup_old_jobs()` 即清除。
...
- `GET /api/trips/saved`
- `POST /api/trips/saved`（完整 `TripState`）
- `GET /api/trips/saved/{trip_id}`
- `POST /api/trips/saved/{trip_id}/replan`（`day`、`new_poi`、可选 `expected_version_id`；冲突返回 409）
- `POST /api/trips/saved/{trip_id}/export/file`（`export_format`、可选 `map_snapshot_base64`）
- `GET /api/trips/saved/{trip_id}/exports`
...
| 保存行程并发编辑 | 行锁 + `expected_version_id` 乐观并发校验；陈旧请求返回 409 | 前端增加冲突刷新与差异提示 |
| 任务存储写入失败 | 默认数据库模式返回持久化错误；JSONL 回退模式写入失败时降级内存态，健康检查端报告 `persistence_error` | 数据库连接监测、可配置回退与自动重试 |
...
| 数据无限增长 | **已实现**：数据库任务年龄/数量清理，导出文件天数/数量清理；启动自动 + REST API + CLI | 可配置清理策略存储 |
...
- 默认数据库模式下持久化失败会返回明确错误；JSONL 回退模式写入失败时才会降级内存态。
